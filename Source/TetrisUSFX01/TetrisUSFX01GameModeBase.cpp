...
void ATetrisUSFX01GameModeBase::BeginPlay()
{
    ...
    for (int i = 0; i <= 4; i++)
    {
        ABoard* SpawnedInventory = GetWorld() -> SpawnActor<ABoard>(ABoard::StaticClass());
        if (SpawnedInventory)
        {
            //If the Spawn succeeds, set the Spawned inventory to the local one and log the success string
            Inventory = SpawnedInventory;
            GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan,FString::Printf(TEXT("%s has been created"), *Inventory->GetName()));
        }
    }
